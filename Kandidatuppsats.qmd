...
This study investigates the prediction of AD/dementia progression using longitudinal biomarker data, comparing traditional and advanced statistical approaches, different biomarkers as well as the number of biomarker measurements per individual. More precisely, the research questions are:
...
<<<<<<< HEAD
First, Cox proportional hazards (Cox PH) models will be used to assess associations between baseline biomarker levels and progression risk. In these models, p-tau181 and NfL will be included either separately or jointly to allow comparison of their individual and combined effects. Second, univariate joint models will be applied to link each biomarker’s longitudinal trajectory to the survival outcome. Third, multivariate joint models will jointly model both biomarker trajectories to capture their combined and potentially correlated effects on disease progression. Finally, latent class joint models will be used to identify subgroups with distinct longitudinal patterns and associated progression risks. Both univariate and multivariate latent class models will be evaluated.
...
=======
To investigate differences between p-tau181 and NfL, the modelling frameworks will be applied separately for each biomarker, followed by a comparative evaluation of their predictive performance and estimated associations with clinical progression. Subsequently, the data will be divided into in-sample data and out-of-sample data, where the in-sample data are used for model development and the out–of-sample data are reserved for model evaluation. This is done to ensure an unbiased assessment of how well the models generalise to unseen data.

Model performance will be evaluated using time-dependent AUC and Brier score for dynamic prediction. For prognostic models predicting progression from MCI to AD dementia, the prediction horizon varies across studies, but a 2-4 year follow-up is most commonly used (Xia et al., 2025, p. 213). This reflects the typical disease course, where a substantial proportion of individuals with MCI convert to dementia within a few years of follow-up; around one-third of individuals with MCI due to AD progress to dementia within five years (Alzheimer’s Association, n.d.). Based on this, a landmark time of 1 year and horizons of 2, 3, 4, 5, and 6 years were selected to capture short- to mid-term progression.
>>>>>>> 93569414dcc3d05282641bac2c790b81676e7277
...
All of the modeling appraoches considered in this study are grounded in the survival analysis framework, where both the outcome of an event and the time from start of follow-up, or baseline time, until it's occurrence are modeled (Ranganathan, Deo & Pramesh, 2025, p. ?). In this analysis, ptau-181 and NfL levels as well as diagnosis status are known at baseline, and the event of interest is dementia diagnosis. Additionally, individuals are either right-censored, meaning that they do not develop dementia during the follow-up period, or they are interval censored, meaning that the exact date of dementia occurrence is not known. This is due to diagnoses being made yearly, and as such the exact date of transition to dementia can be at best approximated.

*As all modeling approaches considered in this study aim to model time to progression to AD, they are grounded in the survival analysis framework. Survival analysis, also known as time-to-event analysis, focuses on both whether an event occurs and the time from the start of follow-up until its occurrence (Ranganathan, Deo & Pramesh, 2025, p. ?). The start of follow-up, also referred to as baseline, is defined as an initial measurement taken at an early time point that serves as a reference for assessing changes over time (NCI, n.d.).*

*Censoring is typically classified as either right or left censoring (Rizopoulos, 2012, p. 36). Right censoring occurs when the event is not observed during the study period, but is known to occur after the last recorded observation, for example due to study end or loss to follow-up. In such cases, the exact event time is unknown but exceeds the last observed time point (Rizopoulos, 2012, p. 36).*
...
While standard joint models assume a homogeneous population, AD is highly heterogeneous, with patients exhibiting different patterns of cognitive decline and biomarker evolution. Joint latent class mixed models (JLCMM) address this by assuming that the population consists of a finite number of unobserved subgroups (latent classes), each characterized by distinct longitudinal trajectories and survival risks.
...
The variable was retrieved from the dataset DXSUM in ADNIMERGE2. For the 1190 included participants, the observations range from 29 September 2005 to 7 November 2025, with the number of diagnostic visits per individual ranging from one to 95.

To define the outcome for the survival analyses, the study focuses on time to incident dementia, where participants are classified as either non-demented or demented. The first recorded diagnosis of dementia is used to define the event, while participants without a dementia diagnosis are censored at their last available visit. This definition is applied consistently in both the standard Cox PH models and the survival submodel (Cox component) of the joint models.
...
P-tau181 is a continuous numerical variable representing one of the two plasma biomarkers used to potentially improve prediction of participants’ progression to dementia. It was retrieved from the dataset UGOPTAU181 in ADNIMERGE2, for which the data was analyzed by the University of Gothenburg (ADNI, n.d.-b). In contrast to the diagnostic data, the p-tau181 measurements are only available between 1 February 2007 and 6 June 2016, meaning that there are diagnostic visits without corresponding p-tau181 measurements. The biomarker measurements are observed 3647 times across the 1190 included participants. However, the unit of measurement is not specified in the dataset documentation.

P-tau181 is used differently across the modelling frameworks. In the standard Cox PH models, baseline p-tau181 values are included as a covariate to assess their association with subsequent risk of progression to AD/dementia. In the joint models, p-tau181 is modelled longitudinally using a LME submodel, where repeated measurements over time are used to describe individual biomarker trajectories. These longitudinal trajectories are then linked to the survival submodel (Cox component), allowing for assessment of how changes in p-tau181 over time are associated with the hazard of progression.

### NfL

NfL, a continuous numerical variable, represents the other plasma biomarker with potential to improve prediction of participants’ progression to dementia. It was retrieved from the dataset "BLENNOWPLASMANFLLONG" in ADNIMERGE2 and the data was collected by the Blennow Lab (ADNI, n.d.-b). Similarly to p-tau181, NfL measurements are available between 1 February 2007 and 6 June 2016, meaning that some diagnostic visits do not have corresponding biomarker measurements. The biomarker is observed 3647 times across the 1191 included individuals. Once again, however, the unit of measurement is not specified in the dataset documentation.

NfL is also used in different ways across the modelling frameworks. In the standard Cox PH models, baseline NfL values are included as a covariate to assess their association with risk of progression to AD/dementia. In the joint models, NfL is modelled longitudinally using a LME submodel based on repeated measurements over time. These trajectories are then linked to the survival submodel (Cox component), allowing for estimation of how longitudinal changes in NfL are associated with the hazard of progression.

### Age

Additionally, the participants’ age is included as a covariate when predicting progression to dementia, to examine how the risk of dementia changes with age. This variable, together with the following variable, were retrieved from the dataset PTDEMOG.
...
In the standard Cox PH models, age at baseline (age_at_baseline) is included as a fixed covariate to adjust for differences in baseline risk of progression to AD/dementia. In the joint models, age at baseline is likewise included as a fixed covariate in the survival submodel (Cox PH component), ensuring appropriate adjustment for baseline age when estimating the association between longitudinal biomarker trajectories and risk of progression.
...
Sex is included as a fixed covariate in all statistical models. In the standard Cox PH models, it is used to estimate sex-specific differences in the risk of progression to AD/dementia, adjusted for other covariates. In the joint models, sex is included as a fixed covariate in both submodels. In the survival submodel (Cox PH component), it accounts for potential differences in the hazard of progression between males and females. In the longitudinal submodel (LME component), it is included as a baseline covariate to account for potential differences in biomarker levels between sexes.
...
APOE-$\epsilon 4$ status is included as a fixed covariate in all statistical models. In the standard Cox PH models, it is used to assess whether genetic risk is associated with progression to AD/dementia. In the joint models, APOE-$\epsilon 4$ status is included as a fixed covariate in both submodels. In the survival submodel (Cox PH component), it allows for estimation of genotype-specific differences in the hazard of progression. In the longitudinal submodel (LME component), it is included as a baseline covariate to account for potential differences in biomarker levels between genotype groups.
...
## Biomarker Distributions and Outliers

The values of p-tau181 ranged from 0.4 to 451.4, with a median of 16.3, a first quartile of 11.1 and a third quartile of 23.6. Similarly, NfL values ranged from 6.4 to 408, with a median of 37.9, a first quartile of 27.8 and a third quartile of 52.5. These descriptive statistics indicate that both biomarkers exhibited pronounced right-skewed distributions, thereby motivating the use of transformation to better satisfy the assumption of normality.

Moreover, when examining observations with large Pearson residuals, specifically those exceeding three standard deviations from the mean, clear individual-level patterns were observed. For example, p-tau181 for one individual increased from 124 to 238 over time, and another individual showed consistently high values, ranging from 110 to 139. Similar patterns were observed for NfL, where one individual increased from 246 to 306, while another remained relatively stable at elevated levels between 121 and 134.8. Accordingly, and given that the values fall within plausible biological ranges, these observations were not treated as outliers but were retained in the analysis. However, due to their magnitude, these values may still have influenced the results and were therefore considered in the interpretation.

Following log-transformation, p-tau181 exhibited a distribution that more closely approximated normality, as shown in Appendix 2. The values became more concentrated around the central region, although a slight residual left-skewness remained. However, this deviation was not considered substantial. Similarly, Appendix 3 demonstrates that the log-transformation of NfL resulted in an even closer approximation to a normal distribution. The spread of values was reduced and observations were more symmetrically distributed around the centre.

Subsequently, the log-transformed variables are considered appropriate for inclusion in the statistical models.

## Correlation Between Biomarkers

The correlation between p-tau181 and NfL was also calculated. The estimated correlation coefficient was 0.247, indicating a weak positive association between the two biomarkers. To be more precise, it suggests that individuals with higher levels of one biomarker tend to have slightly higher levels of the other, although the relationship is not particularly strong. Consequently, the biomarkers share a limited amount of common variation while still providing partly distinct information.

The presence of a positive correlation nevertheless supports the relevance of a multivariate joint modelling framework, as such an approach allows the dependence between the biomarkers to be accounted for while simultaneously evaluating their individual and combined associations with the outcome of interest.
...
## Definition of Baseline Date and Number of Events

The baseline date was defined as earliest available examination date across biomarkers.

**\*\* Add info about how the examination dates for p-tau181 and NfL did not always coincide, and how we handled that**

When restricting the analysis to participants with three or more biomarker measurements, the number of participants who experienced the event of dementia was 177, while the remaining 527 participants were censored. There were also 66 participants who experienced the event at baseline, and these were removed since they had no valid follow-up time and could not contribute to the estimation of time-to-event outcomes.

In comparison, when including participants with one or more biomarker measurements, the number of participants who experienced the event of dementia increased to 198, with 736 participants remaining censored. This larger sample therefore captures more observed dementia events, although some participants contributed fewer repeated biomarker observations over time.

## Division Into In And Out-Datasets

The analyzed data were split so that the in-dataset comprised three quarters of the total data and the out-dataset comprised one quarter.

When limiting the participants to those with three or more measurements of the biomarkers, the sample size of the in-dataset was 528, among which 128 experienced the event of dementia, corresponding to 24.24%, while 400 individuals were censored. In the out-dataset, there were 176 individuals, among whom 49 experienced the event of dementia, corresponding to 27.84%, while 127 individuals were censored.

In comparison, when restricting the analysis to participants with at least one biomarker measurement, the in-dataset consisted of 701 individuals, among whom 145 experienced the event of dementia, corresponding to 20.69%, while 556 individuals were censored. The out-dataset included 233 individuals, among whom 53 experienced the event of dementia, corresponding to 22.75%, while 180 individuals were censored.

# Results

*Present the results from our analyses. Do not interpret results here! That should be done in the discussion.*

The results are structured in two parts. Part 1 includes participants with three or more biomarker measurements, while Part 2 repeats the same analyses including all participants with at least one biomarker measurement.

## Including Participants With Three or More Biomarker Measurements
...
**Table 1: p-values for the proportional hazards assumption tests after exclusion of age at baseline in the Cox PH models.**
...
**Table 2: hazard ratios with 95% confidence intervals and p-values from the Cox PH models including baseline biomarkers**
...
Initial compariosn of model performance by assessment of the concordance indices for all cox models was done. The results presented in **table ?** show that the concordance index evaluates all three models similarly.

**Table ?: concordance indices for all fitted Cox PH models with baseline biomarker values**

```{r}
knitr::kable(readRDS("tables/c_index_table.rds")) %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```

### Joint Models

The evaluation of the association between biomarkers and progression to dementia due to AD, while adjusting for relevant covariates, was further extended by fitting joint models that incorporated the longitudinal trajectories of p-tau181 and NfL with the time-to-diagnosis data. For the longitudinal component, an LME model was fitted separately for p-tau181 and NfL.

Plotting the residuals against the fitted values for the LME model of p-tau181 showed no clear evidence of heteroskedasticity, as shown in Appendix 4. The slight deviations in the tails of the Q-Q plot, shown in Appendix 5, were also not considered to be of concern. A similar pattern was observed for the LME model of NfL, where the plot of residuals against fitted values in Appendix 6 showed even less evidence of heteroskedasticity compared to the corresponding plot for p-tau181. The Q-Q plot for NfL in Appendix 7 showed similar results to those observed for p-tau181. Generalised linear mixed effects models with different link functions were also fit and their residuals analysed, however none of these models demonstrated a better fit than the log-transformed model. Consequently, these were considered to provide an adequate fit to the data, suggesting that they are suitable for use within the joint modeling framework.
...
**Figure 1: trace plot of the** $α$**-parameter for the joint model of p-tau181.**

![](figures/convergence_plot_ptau.png){width="400"}

The trace plot shows that the three MCMC chains fluctuate around a stable mean value throughout the iterations, without any clear trend or drift. This suggests that the chains have reached a stable region. In addition, the chains overlap well and mix with each other, indicating that they are sampling from the same posterior distribution. Although some random variation is present, this is expected in MCMC sampling. Overall, the plot supports good convergence. A similar pattern was observed for the joint model of NfL, as shown in Figure 2.

**Figure 2: trace plot of the** $α$**-parameter for the joint model of NfL.**

![](figures/convergence_plot_nfl.png)

The chains likewise fluctuated around a stable mean value with good mixing, indicating convergence of the $α$-parameter comparable to the joint model of p-tau181. Finally, the multivariate joint model also exhibited a similar overall behaviour, as shown in Figure 3.

**Figure 3: trace plot of the** $α$**-parameter for the multivariate joint model**

![](figures/convergence_plot_multi.png)

Compared to the single-biomarker models, the trace plot for the multivariate joint model shows a slightly wider range of $α$-values, indicating a somewhat larger variability in the $α$-parameter. This suggests increased uncertainty in the parameter estimates, which is expected given the higher complexity of the model. Despite this, the chains still fluctuate around a stable mean value with no visible trend or drift, and they continue to overlap and mix well. Thus, all three models appear to have converged adequately.
...
**Table 3:** $\widehat{\mathbf{R}}$ **values of the survival outcome in the joint models**
...
**Table 4:** $\widehat{\mathbf{R}}$ **values of the longitudinal outcome in the joint models**
...
**Table 5: hazard ratios with 95% confidence intervals and p-values from the joint models**
...
For all models, a Weibull baseline hazard model was chosen and checked against whether a flexible baseline hazard with the same number of parameters resulted in a lower BIC. The results were verified using the $gridsearch()$ function which allows for better exploration of the parameter space to ensure finding a global instead of a local maximum solution.

Firstly, latent class mixed models were fit on the longitudinal ptau-181 and NfL measurements in order to assess whether latent classes could be distinguished from the biomarker trajectories. None of these models could adequately separate the sample population into meaningful latent classes, suggesting that ptau-181 and NfL biomarker trajectories alone do not hold enough information to be able to distinguish heterogenous disease sub-types. Multivariate latent class models were estimated on ptau-181 and NfL measurements simultaneously. Again, this model could not discriminate valid latent classes. Interestingly, incorporating the survival data resulted in JLCMMs that could meaningfully classify individuals into latent classes, the results of which follow below.

**JLCMM PTAU**

The random effects structure was decided by a likelihood ratio test between a one class model with random intercept and one with both a random intercept and slope, as well as analysis of the BIC. Despite a non-significant test statistic of the likelihood ratio test ($\chi^2 = 1.38, p = 0.50$), a random effects structure with only intercept was chosen due to a lower BIC for the simpler model ($BIC = 4077$) compared with the complex model ($BIC = 4088$). Next, JLCMMs were fit with up to four classes and analysis of their log-likelihoods, BIC and entropy follow.

**Table 6: log-likelihood, BIC and entropy for JLCMMs with longitudinal ptau-181 measurements**

```{r}
knitr::kable(readRDS("tables/summary_jlcmm_ptau.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

The results show that both log-likelihood and entropy increase with additional latent classes, yet the BIC is minimised for the model with two latent classes. Given the inconclusive results from this comparison, the posterior probabilities were additionally analysed in order to decide the number of latent classes.

The posterior classification table shows the number of individuals assigned to each class, whereas the mean posterior probabilities show, for individuals in each class, the average probability of being assigned to each class. For example, values for $class1$, $prob1$ close to $1$ and $class2$, $prob1$ close to $0$ show good class discrimination.

**Table 7: posterior classification table and mean posterior probabilities for the two class JLCMM for longitudinal ptau-181 measurements**

```{r}
knitr::kable(readRDS("tables/postprob_m1_ptau.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

**Table 8: posterior classification table and mean posterior probabilities for the three class JLCMM for longitudinal ptau-181 measurements**

```{r}
knitr::kable(readRDS("tables/postprob_m2_ptau.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

For the two class model (table 7), the class sizes are reasonable and the mean posterior probabilities show good class discrimination, meaning the model has high certainty in assigning individuals to their respective classes. Comparison with the three class model (table 8) shows somewhat poorer class discrimination as well as a questionably small $class3$, with only $3.79\%$ of individuals being assigned to this class. Therefore, the two class model is chosen as the best model for most accurately discerning latent classes from the longitudinal ptau-181 measurements and the time-to-dementia data.
...
Figure 4 shows the class-specific survival curves. The probability of remaining dementria-free reduces sharply for $class2$ up until about 5 years, where the curve starts to level out. For individuals in $class1$, the probability of dementia diagnosis remains close to 0 for about 10 years, where the probability of diagnosis increases slowly at first and then, from about 15 years, increases sharply.

**JLCMM NFL**

Again, the results of a likelihood ratio test between two models was used to determine the random effects structure. This resulted in a significant test statistic ($\chi^2 = 10.5, p = 0.005$), evidence that the simpler model with only a random intercept was a better fit for the data.

**Table 10: log-likelihood, BIC and entropy for JLCMMs with longitudinal NfL measurements**

```{r}
knitr::kable(readRDS("tables/summary_jlcmm_nfl.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

Analysis of the log-likelihood, BIC and entropy provided in table 10 suggests that the two class model in this instance provides the best method for discriminating latent classes given longitudinal NfL measurements and time-to-diagnosis data. The log-likelihood increases most between the one class and the two class model, and only increases very slightly between the two class and the three class model. The BIC is minimised and entropy is maxmised for the two class model. The posterior probabilities are also analysed in order to ensure good class discrimination.

**Table 11: posterior classification table and mean posterior probabilities for the two class JLCMM for longitudinal NfL measurements**

```{r}
knitr::kable(readRDS("tables/postprob_m1_nfl.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

Latent class sizes for the two class model are reasonable, and good class discrimination is shown when assessing average posterior probabilities (table 11). This is evidence for the two class model and that the data supports two latent classes.
...
Figure 6 shows the class-specific survival given from the two class JLCMM with longitudinal NfL measurements. The plot shows similar survival curves to those in figure 5, suggesting both models have discerned the same latent classes.

**MULTIVARIATE JLCMM**

The ptau-181 and NfL measurements were also modeled simultaneously with a multivariate JLCMM. Models with one, two and three classes were fit with a simple random effects structure as although the BIC was reduced with a random intercept and slope, the difference was minimal at \~ $1.1$ unit decrease. Although the log-likelihood decreased with increasing number of classes, the biggest decrease was between the one and two class models. Additionally, the BIC was minimised and the entropy was maximised for the two class model.

**Table 10: log-likelihood, BIC and entropy for JLCMMs with longitudinal ptau-191 and NfL measurements**

```{r}
knitr::kable(readRDS("tables/summary_jlcmm_ptau_nfl.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

Assessment of the posterior probabilities for the two class model shows both reasonable class sizes and good class discrimination.

**Table 11: posterior classification table and mean posterior probabilities for the two class JLCMM for longitudinal ptau-191 and NfL measurements**

```{r}
knitr::kable(readRDS("tables/postprob_ptau_nfl.rds")) %>%
  kable_styling(latex_options = "hold_position")
```

## Including Participants With One or More Biomarker Measurements

### Cox PH Models

**\*\*Lägga till kort om att PH assumption fortfarande såg bra ut, att age at baseline fortfarande bröt mot antagandet och därför togs bort även här.**

When comparing individuals with three or more measurements of p-tau181 and NfL to those with at least one measurement, we observe that the inclusion of individuals with fewer repeated observations primarily increases sample size and statistical power, while having limited impact on the direction of the estimated effects for the strongest biological predictors.

Across both model specifications, APOE-$ε4$ remains the strongest and most stable genetic predictor of progression. Hazard ratios are very similar, with a slight increase in the larger cohort, from about 2.0 to 2.2 for one allele and from about 3.6 to just above 4.0 for two alleles. The confidence intervals are of similar width across both analyses, suggesting stable uncertainty in the estimates and consistent precision.

For the biomarkers, both ptau and NfL are consistently associated with progression, but NfL shows stronger effects and slightly lower uncertainty than ptau across both cohorts. For example, in single-biomarker models, NfL shows higher hazard ratios than ptau and generally slightly narrower confidence intervals, indicating more precise estimation of its association.

When both biomarkers are included simultaneously, the same pattern remains, with NfL consistently showing higher hazard ratios than ptau in both model specifications. Effect sizes are slightly lower compared to single-biomarker models, and confidence intervals are somewhat wider, consistent with shared variance between biomarkers, but the relative ranking between ptau and NfL is unchanged.

In contrast, sex shows no association with progression in any of the models, ith hazard ratios close to one and confidence intervals of similar width across both cohorts, indicating stable but non-informative estimates.

### Joint Models

**\*\*Lägga till kort om att residualerna och QQ-plots inte ändrades drastikt, att convergence fortfarande ser bra ut.**

When extending the comparison to the joint models, a similar overall pattern is observed, with consistent findings across model specifications and stable confidence interval widths, indicating preserved precision across analyses. There is a clearer separation in effect magnitudes between biomarkers, while uncertainty remains broadly comparable between models.

In the joint models, APOE-$ε4$ remains a strong and stable predictor of progression. Hazard ratios are slightly higher in the model including all individuals, increasing from about 1.9 to 2.2 for one allele and from about 3.2 to 3.5 for two alleles, with a further increase to around 4.3 for two alleles in the full model. Confidence intervals remain well separated from one and show substantial overlap between model specifications, supporting the robustness of the genetic association within a joint modeling framework.

For the longitudinal biomarkers, a clearer separation between ptau and NfL is observed. In both model settings, NfL consistently shows a stronger association with progression than ptau. In the first joint model, ptau has a hazard ratio around 3.3, while NfL is around 3.5, and in the more inclusive model these increase to approximately 4.5 for ptau and 5.5 for NfL. NfL also tends to show slightly narrower confidence intervals than ptau, indicating lower uncertainty in its estimated association.

When both biomarkers are included simultaneously, the same pattern remains, with NfL consistently showing higher hazard ratios than ptau in both joint model specifications. Effect sizes are slightly lower compared to models with single biomarkers, and confidence intervals are correspondingly wider, consistent with shared variance between biomarkers.

Sex also remains non-significant across all joint models, with no meaningful differences between model specifications.

### Joint Latent Class Mixed Models

The JLCMM with longitudinal ptau-181 measurements
...
**Table ?: Time-dependent AUC by model and follow-up time point**
...
**Table ?: Time-dependent Brier score by model and follow-up time point**
...
Despite this, interval-censoring was not implemented given the level of this thesis and its time constraints. Properly accounting for interval-censored event times would require more complex statistical methods, for which standard Cox PH models would no longer be directly applicable. **This pragmatic approximation is also commonly used in previous literature (källa), which supports the approach taken in this study.**
...
## Appendix 1: p-values for the proportional hazards assumption tests for all covariates in the Cox PH models
...
## Appendix X: results from the likelihood ratio test for the Cox PH models with and without gender as a covariate

```{r}
knitr::kable(readRDS("tables/surv_lrt_table.rds"))  %>%
  kable_styling(latex_options = c("hold_position", "scale_down"))
```

## Appendix 2: distribution of p-tau181 after log-transformation

![](figures/hist_log_p_tau.png){width="400"}

## Appendix 3: distribution of NfL after log-transformation

![](figures/hist_log_nfl.png){width="400"}

## Appendix 4: fitted values versus residuals from the LME model for p-tau181

![](figures/residuals_vs_fitted_ptau.png){width="400"}

## Appendix 5: Q-Q plot of the LME model for p-tau181

![](figures/qq_ptau.png){width="400"}

## Appendix 6: fitted values versus residuals from the LME model for NfL

![](figures/residuals_vs_fitted_nfl.png){width="400"}

## Appendix 7: Q-Q plot of the LME model for NfL
