...
The variable was retrieved from the DXSUM dataset in ADNIMERGE2. For the 1190 included participants, observations range from 29 September 2005 to 7 November 2025, with the number of diagnostic visits per individual ranging from one to 95.

Since the study focuses on time to incident dementia, participants are classified as either non-demented or demented for the survival analyses, without distinguishing between CN and MCI states.
...
P-tau181 is a continuous variable representing one of the two plasma biomarkers used to potentially improve prediction of progression to dementia. It was retrieved from the UGOPTAU181 dataset in ADNIMERGE2, which has been processed by the University of Gothenburg (ADNI, n.d.-b).

In contrast to the diagnostic data, p-tau181 measurements are only available between 1 February 2007 and 6 June 2016, meaning that some diagnostic visits lack corresponding biomarker measurements. The dataset contains 3647 p-tau181 measurements across 1190 participants. However, the unit of measurement is not specified in the dataset documentation.

### NfL

NfL is a continuous variable representing the second plasma biomarker included in the analyses, with potential to improve prediction of progression to dementia. It was retrieved from the BLENNOWPLASMANFLLONG dataset in ADNIMERGE2, and the data were collected by the Blennow Lab (ADNI, n.d.-b).

Similar to p-tau181, NfL measurements are available between 1 February 2007 and 6 June 2016, resulting in some diagnostic visits without corresponding biomarker measurements. The dataset contains 3647 NfL measurements across the 1190 included participants. As for p-tau181, the unit of measurement is not specified in the dataset documentation.

### Age

Additionally, age is included as a covariate when predicting progression to dementia, as risk of dementia is known to increase with age. The variable was retrieved from the PTDEMOG dataset and is initially recorded as year of birth for each participant, ranging from 1915 to 1971 among the 1190 included participants. It is then transformed into age at visit and age at baseline, which are more suitable for the analyses.

### Gender

Another covariate included when predicting progression to dementia is participants’ sex at birth, as recorded on their original birth certificate. This is included due to reported differences in disease risk and progression between sexes. This binary variable includes two categories: male and female. Among the 1190 included participants, 543 were female and 647 were male.
...
```{r, echo=FALSE}
kable_standard(readRDS("tables/table_cox.rds"))
```
...
```{r, echo=FALSE}
kable_standard(readRDS("tables/table_jm.rds"))
```
...
```{r}
kable_standard(readRDS("tables/postprob_m1_ptau.rds"))
```
...
```{r}
kable_standard(readRDS("tables/postprob_m2_ptau.rds"))
```
...
```{r}
kable_standard(readRDS("tables/postprob_m1_nfl.rds"))
```
...
```{r}
kable_standard(readRDS("tables/postprob_ptau_nfl.rds"))
```
...
## Appendix X: summary of fit statistics for JLCMM with 1+ longitudinal measurements for ptau-181

```{r}
kable_standard(readRDS("tables/summary_jlcmm_ptau_full.rds"))
```

## Appendix X: summary of posterior probabilities for the two class JLCMM with 1+ longitudinal measurements for ptau-181

```{r}
kable_standard(readRDS("tables/pprob_m1_ptau_full.rds"))
```

## Appendix X: summary of posterior probabilities for the three class JLCMM with 1+ longitudinal measurements for ptau-181

```{r}
kable_standard(readRDS("tables/pprob_m2_ptau_full.rds"))
```

## Appendix X: summary of fit statistics for JLCMM with 1+ longitudinal measurements for NfL

```{r}
kable_standard(readRDS("tables/summary_jlcmm_nfl_full.rds"))
```

## Appendix X: summary of posterior probabilities for the two class JLCMM with 1+ longitudinal measurements for NfL
...
## Appendix X: summary of fit statistics for JLCMM with 1+ longitudinal measurements for ptau-181 and NfL

```{r}
kable_standard(readRDS("tables/summary_jlcmm_ptau_nfl_full.rds"))
```

## Appendix X: summary of posterior probabilities for the two class JLCMM with 1+ longitudinal measurements for ptau-181 and NfL
