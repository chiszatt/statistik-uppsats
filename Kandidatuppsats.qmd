...
The modelling approaches will be applied to data obtained from the ADNI dataset and handled in accordance with their data use and access policies (ADNI, n.d.-b). **In this context, it is also worth noting the limited availability of AD data in Sweden due to legal and regulatory constraints governing access to health data. (i think this is the case in most countries, it isn't freely available)** This limitation further increases the importance of utilizing well-characterized international datasets such as ADNI, as they provide the necessary longitudinal and high-quality data required to conduct the types of analyses performed in this study.
...
Following log-transformation, p-tau181 exhibited a distribution that more closely approximated normality, as shown in Appendix 1. The values became more concentrated around the central region, although a slight residual left-skewness remained. However, this deviation was not considered substantial. Similarly, Appendix 2 demonstrates that the log-transformation of NfL resulted in an even closer approximation to a normal distribution. The spread of values was reduced and observations were more symmetrically distributed around the centre.
...
To be more precise, a longitudinal submodel was specified for each biomarker together with a survival submodel, the latter taking the form of a standard Cox model. As indicated in Table 2, the proportional hazards assumption was satisfied for this model. For the longitudinal component, an LME model was fitted separately for p-tau181 and NfL.

Plotting the residuals against the fitted values for the LME model of p-tau181 showed no clear evidence of heteroskedasticity, as shown in Appendix 3. The slight deviations in the tails of the Q-Q plot, shown in Appendix 4, were also not considered to be of concern. A similar pattern was observed for the LME model of NfL, where the plot of residuals against fitted values in Appendix 5 showed even less evidence of heteroskedasticity compared to the corresponding plot for p-tau181. The Q-Q plot for NfL in Appendix 6 showed similar results to those observed for p-tau181. Consequently, both models were considered to provide an adequate fit to the data, suggesting that they are suitable for use within the joint modeling framework.
...
Additionally, convergence of the models was assessed by examining the $\widehat{R}$ values of the survival outcome and the longitudinal outcome in each joint model, with the survival $\widehat{R}$ values presented in Table 4.

**Table 4:** $\widehat{\mathbf{R}}$ **values of the survival outcome in the joint models**
...
While Table 4 presents the $\widehat{R}$ values for the survival outcomes, the corresponding $\widehat{R}$ values for the longitudinal outcomes in each joint model are shown in Table 5.

**Table 5:** $\widehat{\mathbf{R}}$ **values of the longitudinal outcome in the joint models**
...
Subsequently, the hazard ratios from the joint models were estimated and are presented in Table 6, along with 95% confidence intervals and p-values.
...
**Table 7: log-likelihood, BIC and entropy for JLCMMs with longitudinal ptau-181 measurements**
...
**Table 8: posterior classification table and mean posterior probabilities for the two class JLCMM for longitudinal ptau-181 measurements**
...
**Table 9: posterior classification table and mean posterior probabilities for the three class JLCMM for longitudinal ptau-181 measurements**
...
For the two class model (table 8), the class sizes are reasonable and the mean posterior probabilities show good class discrimination, meaning the model has high certainty in assigning individuals to their respective classes. Comparison with the three class model (table 9) shows somewhat poorer class discrimination as well as a questionably small $class3$, with only $3.79\%$ of individuals being assigned to this class. Therefore, the two class model is chosen as the best model for most accurately discerning latent classes from the longitudinal ptau-181 measurements and the time-to-dementia data.
...
Figure ? provides an initial overview of the time-dependent AUC of the Cox models across the follow-up period.

**Figure ?: Time-dependent AUC curves for the Cox models**

![](figures/auc_plot_cox.png){width="400"}

Overall, all Cox models demonstrated relatively stable predictive performance over time, with AUC values consistently approximately between 0.80 and 0.875, indicating good discriminative ability throughout the study period.

The Cox model including both baseline biomarker values showed the highest predictive performance across all evaluated time points. In contrast, the single-marker models demonstrated slightly lower and somewhat more variable performance over time. The trajectories of the Cox NfL and Cox ptau-181 models were broadly similar, although the NfL-only model appeared to perform slightly better during later follow-up.
...
As expected, Table ? demonstrates that predictive performance is highest at two-year follow-up, particularly for joint models including multiple biomarkers. In contrast, models based on single biomarkers show lower performance, with Cox models based on baseline biomarkers performing least well. At later follow-up time points, differences between models are less pronounced, with AUC values converging to a narrower range between approximately 0.77 and 0.87, reflecting more similar performance across models.

Additionally, predictive performance was evaluated using the time-dependent Brier score, with Figure ? providing an initial overview of the Brier scores for the Cox models across the follow-up period.

**Figure ?: Time-dependent Brier score curves for the Cox models**

![](figures/Brier_plot_cox.png){width="400"}

Overall, the Brier scores remain low for all Cox models throughout the follow-up period, indicating good overall predictive accuracy. At the two-year follow-up, all models demonstrate their lowest Brier scores, around 0.05. However, a gradual increase in Brier score is observed over time, reaching approximately 0.15 at six years, suggesting that predictions become less precise at longer follow-up times.

Differences between the Cox models are relatively small across the follow-up period. A slight difference appear at the later follow up-points, where the Cox model including baseline values of both biomarkers shows a slightly lower Brier score than the single-biomarker models. The two single-biomarker models display very similar trajectories over time.
...
Similar to the Cox models, the Brier scores are lowest at earlier follow-up times and increase gradually over time. At the two-year follow-up, however, the latent class joint models show slightly lower Brier scores than the other joint models and the Cox models. Differences between the joint models are otherwise small and become less pronounced over follow-up time, with Brier scores converging to around 0.15 across models at six years.
...
## Appendix 1: distribution of p-tau181 after log-transformation

![](figures/hist_log_p_tau.png){width="400"}

## Appendix 2: distribution of NfL after log-transformation

![](figures/hist_log_nfl.png){width="400"}

## Appendix 3: fitted values versus residuals from the LME model for p-tau181

![](figures/residuals_vs_fitted_ptau.png){width="400"}

## Appendix 4: Q-Q plot of the LME model for p-tau181

![](figures/qq_ptau.png){width="400"}

## Appendix 5: fitted values versus residuals from the LME model for NfL

![](figures/residuals_vs_fitted_nfl.png){width="400"}

## Appendix 6: Q-Q plot of the LME model for NfL
