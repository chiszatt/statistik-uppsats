...
The predictive performance of these modelling frameworks will be compared by time-dependent AUC and Brier scores, ***as well as the estimated associations of p-tau181 and NfL with disease progression***. For prognostic models predicting progression from MCI to AD-dementia, a 2–4 year follow-up is most commonly used (Xia et al., 2025, p. 213). This reflects typical disease progression, where around one-third of individuals with MCI due to AD develop dementia within five years (Alzheimer’s Association, n.d.). Based on this, a landmark time of 1 year and prediction horizons of 2–6 years will be used to capture short- to mid-term progression. To ensure an unbiased assessment of model performance and generalisability, in-sample data is used for model development with three quarters of the sample population, whilst out-of-sample data was reserved for model evaluation with the remaining quarter.
...
JLCMMs address suspected disease heterogenity by assuming that the sample population consists of a finite number of unobserved subgroups (latent classes), each characterized by distinct longitudinal trajectories and survival risks (Kyheng, Babykina & Duhamel, 2025, p. 2). Individuals within latent classes are assumed to share longitudinal biomarker trajectories and survival risks (Proust-Lima et al., 2018, p. 5) and the classes are not described by the observed covariates (Rizopoulos, 2012, p. 122).
...
where $\mathbf{X}_i$ is the set of covaritates for the $i^{th}$ individual and $\boldsymbol{\beta}_g$ is the set of coefficients for the $g^{th}$ group. Given there are no prior assumptions about the latent classes, no covariates are specified in the multinomial logistic regression model (Kyheng, Babykina & Duhamel, p. 2). The class submodel then simplifies to:
...
All parameters in all of the candidate models were significant ($p < 0.001$) and the results are presented in the **appendix (X).** First, the fit statistics of the model using longitudinal p-tau181 measurements are presented below (**table ?**). Log-likelihood and entropy increased with additional latent classes, however the BIC was minimised for the model with two latent classes.
...
Among the models including p-tau181, the time-dependent AUC values remain above 0.75 across all three modelling approaches throughout the follow-up period, indicating stable and relatively strong predictive performance overall. At earlier time points, the joint model (JM) yields the highest AUC values. However, the differences between the models decrease over time, and at the six-year follow-up, the latent class joint model (LCJM) yields slightly higher values compared with the other two models. The Cox PH model consistently yields the lowest AUC values.

Among the models including NfL, a similar overall pattern is observed. As shown in Appendix X, the models maintain AUC values above 0.75 throughout the follow-up period, indicating consistently strong predictive performance. At earlier follow-up times, the joint model also yields the highest AUC values. These values then decline over time, becoming more similar to those of the other models. Moreoever, at the six-year follow-up, the latent class joint model (LCJM) achieves the highest AUC value.
...
For instance, by showing that the JM among the models including p-tau181 has an AUC value of 0.902 at the two-year follow-up, while the corresponding Cox PH model has an AUC value of 0.823 and the corresponding latent class joint model has an AUC value of 0.862, Table X confirms the results previously observed in the figure presenting the time-dependent AUC curves for these models. The differences between the models then decrease over time; at the four-year follow-up, the AUC values are 0.802 for the Cox PH model, 0.806 for the JM, and 0.811 for the LCJM.
...
Among the models including p-tau181, the time-dependent Brier score values remain low, approximately between 0.05 and 0.16, across all three modelling approaches throughout the follow-up period. This indicates stable and relatively strong predictive performance overall. At earlier follow-up points, however, the latent class joint model yields lower Brier score values compared with the other two models, indicating better predictive performance. The differences then become smaller at later follow-up times, where the models show similar Brier score values.

A similar overall pattern is also observed among the models including NfL; as, shown in Appendix X, the latent class joint model yields lower Brier score values compared with the other two models at earlier follow-up points. Then, at later follow-up times, these values become smaller, resulting in the differences becoming smaller between the models, with values being low and around the same range as for models including p-tau181.

The same can be said for the time-depending Brier score values among the models including both biomarkers simultaneously; the Cox PH model and the joint model yield similar, stable AUC values across the follow-up period, with low values approximately between 0.05 and 0.14.

As for the time-depending AUC results, the time-depending Brier score results are also confirmed by Table X containing the Brier score values for each modelling approach and at each follow-up time point.
...
Within the Cox PH models, the combined biomarker approach generally yields the highest discrimination compared to single biomarker models. At earlier time points, Cox models based on NfL tend to perform slightly better than p-tau181, but both single-marker models are consistently outperformed by the combined model (Cox both), which maintains the highest AUC across most follow-up times. Both single-biomarker models show a gradual decline in performance over time, with relatively similar trajectories.
...
As when comparing the different modelling approaches, these results are also indicated by Table X, containing the time-dependent AUC values for each biomarker specification and at each follow-up time point. For example, at the two-year follow-up within the joint modelling framework, the combined model achieved an AUC value of 0.967, compared to an AUC value of 0.953 for JM NfL and 0.902 for JM p-tau181, confirming that the combined joint model provides the highest predictive performance at this time point.

Additionally, the time-dependent Brier score curves showed similar results across the different biomarker specifications within the Cox PH models.

**Figure X: time-depending Brier score curves for different biomarker specifications (Cox PH models)**

<!-- ![](figures/q2_brier_cox.png) -->

Within the Cox PH models, the combined biomarker approach consistently achieves the lowest Brier scores across most time points, indicating improved calibration and reduced prediction error compared to single-biomarker models. NfL-based Cox models generally perform slightly better than p-tau181 at earlier time points, but the differences between single biomarkers remain modest. Both single-biomarker models show increasing Brier scores over time, reflecting a gradual decline in predictive accuracy, while the combined model maintains a consistently lower error across follow-up.

Similarly, Appendix X shows that, within the joint modelling framework, the combined JM approach again performs best, showing the lowest prediction error across all time points. When considered separately, NfL-based joint models tend to achieve slightly lower Brier scores than p-tau181-based models at earlier time points, while both remain clearly outperformed by the combined biomarker model. Although prediction error increases over time for all JM-based approaches, they maintain relatively strong calibration throughout the follow-up period.

Within the latent class joint models, ....

Table X containing the time-dependent Brier score values for each biomarker specification and at each follow-up time point confirms these results. For instance, within the Cox PH models at the two-year follow-up, the combined biomarker approach has a Brier score of 0.0520, while Cox p-tau181 has a value of 0.0542 and Cox NfL has a value of 0.0535, confirming that the combined model yields the lowest prediction error at this time point.
...
The predictive performance of these models was assessed by comaparing time-dependent AUC and Brier scores with a landmark time of 1 year over horizons spanning 1-5 years. Modest differences in AUC were reported between the Cox PH models with single biomarker baseline values and the model using both p-tau181 and NfL at baseline, suggesting minimal increase in discriminative value over modeling either biomarker by itself.

The multivariate joint model performed best at the 2 year time point, showing better early discrimination than the other models. The differences were mostly modest, with the smallest difference being 0.01 when compared to the joint model with longitudinal NfL alone. The largest difference was 0.156 when compared to the Cox PH with baseline p-tau181 values, suggesting clearly superior early predictions from the multivariate joint model. This is clinically meaningful, as early identification of those at risk of AD-dementia could be beneficial in clinical trial enrolement. At the 6 year mark, the differences in AUC between models were reduced. The multivariate joint model again presented the highest AUC, suggesting that despite the expected reduction in predictive performance over time, multivariate joint modeling of plasma p-tau181 and NfL may be justified even when long-term discrimination between cases and non-cases are of importance.
...
When comparing Brier scores, the short-term predictive performance of the JLCMMs were superior when compared against the other models. The JLCMMs explained around 88% more of the variance than the null model which assigns all individuals the average event probability of 0.28. This was around 72% for all the Cox PH models and 73 % for the joint models. This suggests the JLCMM predicts individual survival probabilities closer to the true event probabilities.

The predictive performance of plasma p-tau181 and NfL were compared in both univariate and multivariate models by again comparing AUC and Brier scores. For p-tau181, AUC was lower than for NfL across all models at the 2 year mark. Differences were modest for the Cox PH models, whilst clearer distinctions were found for the joint models and JLCMMs, suggesting predictive value is gained mostly from NfL trajectories rather than single-point measurements. Over time, differences between the predictive performance of the biomarkers reduced, however NfL performed better across most time points. Comparison of Brier scores gave similar results between biomarkers.

Finally, the effect of the inclusion of individuals with sparse biomarker data was assessed by a precision analysis where the results of models estimated on individuals with at least three measurements were compared with those fit on a cohort of inidivudals with at least one measurement.
...
## Appendix X: trace plots of the $α$-parameters for the multivariate joint model
