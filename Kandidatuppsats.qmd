...
For instance, by showing that the JM among the multivariate models has an AUC value of 0.967 at the two-year follow-up and an AUC value of 0.846 at the six-year follow-up, Table x confirms that it provides the best overall predictive performance among these models.

By evaluating the predictive performance through time-depending Brier score instead, the patterns remain the same, but with lower values indicating better predictive accuracy, as shown by Figure X ; it presents the time-dependent Brier score curves for the different modelling approaches, with the biomarker being fixed as p-tau181.
