...
Latent class mixed models were fit on the longitudinal ptau-181 and NfL measurements in order to assess whether the model can distinguish latent classes from the biomarker trajectories alone. None of these models could adequately separate the sample population into meaningful latent classes, suggesting that ptau-181 and NfL biomarker trajectories alone do not hold enough information to be able to distinguish heterogenous disease sub-types. Multivariate latent class models were estimated on ptau-181 and NfL measurements simultaneously. Again, the models could not discriminate valid latent classes from this data alone. Interestingly, incorporating the survival data resulted in JLCMM's that could meaningfully classify individuals into latent classes.

**JLCMM PTAU**

The random effects structure was decided by a likelihood ratio test between a one class model with random intercept and one with both a random intercept and slope, as well as analysis of the BIC. Despite a non-significant test statistic of the likelihood ratio test ($\chi^2 = 1.38, p = 0.50$), a random effects structure with only intercept was chosen due to lower BIC for the simpler model ($BIC = 4077$) compared with the complex model ($BIC = 4088$).
...
The results show that both log-likelihood and entropy increase with additional latent classes, yet the BIC is minimised for the model with two latent classes. Given the inconclusive results from analysis of log-likelihood, BIC and entropy for the four models, the posterior probabilities were additionally analysed in order to decide the number of latent classes.

The posterior classification table shows the number of individuals assigned to each class, whereas the mean posterior probabilities show, for inidividuals in each class, the average probability of being assigned to each class. For example, values for $class1$, $prob1$ close to $1$ and $class2$, $prob1$ close to $0$ show good class discrimination.
...
For the two class model, the class sizes are reasonable and the mean posterior probabilities show good class discrimination, meaning the model has high certainty in assigning individuals to the two latent classes. Comparison with the three class model shows somewhat poorer class discrimination as well as a questionably small $class3$, with only $3.79\%$ of individuals being assigned to this class. Therefore, the two class model is chosen as the best model for most accurately discerning latent classes.

**Figure 4: class-specific survival curves for the two class JLCMM for longitudinal ptau-181 measurements**

![](figures/class-specific-survival-jlcmm-ptau.png){width="400"}

**Figure 5: class-specific ptau-181 trajectories for the two-class JLCMM**

![](figures/longitudinal-jlcmm-ptau.png){width="400"}

**JLCMM NFL**

Again, a the results of a likelihood ratio test between two models was used to determine the random effects structure. This resulted in a significant test statistic ($\chi^2 = 10.5, p = 0.005$), evidence that the simpler model with only a random intercept was a better fit for the data.
...
Latent class sizes for the two class model are reasonable, and good class discrimination is shown when assessing average posterior probabilities. This is evidence for the two class model and that the data supports two latent classes.

**Figure 6: class-specific survival curves for the two class JLCMM for longitudinal NfL measurements**

![](figures/class-specific-survival-jlcmm-nfl.png){width="400"}

**Figure 7: class-specific NfL trajectories for the two class JLCMM**

![](figures/longitudinal-jlcmm-nfl.png){width="400"}
