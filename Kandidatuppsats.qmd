...
The random effects structure was decided by a likelihood ratio test between one class model with random intercept and one with both a random intercept and slope, as well as analysis of the BIC. Despite a non-significant test statistic of the likelihood ratio test ($\chi^2 = 1.38, p = 0.50$), a random effects structure with only intercept was chosen due to lower BIC for the simpler model ($BIC = 4077$) compared with the complex model ($BIC = 4088$).

The log-likelihoods, BIC and entropy of the one, two, three and four class JLCMM for ptau-181 are presented below:
...
The log-likelihoods, BIC and entropy of the one, two and three class JLCMM's for NfL were as follows:
...
**Figure ?: Time-dependent AUC curves for the joint models and latent class joint models**
...
Figure 2 presents the corresponding time-dependent Brier scores for the joint models and latent class joint models.
