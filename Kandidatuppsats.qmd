...
The random effects structure was decided by a likelihood ratio test between one class model with random intercept and one with both a random intercept and slope. This resulted in a significant test statistic ($\chi^2 = 10.5, p = 0.005$), evidence that the simpler model was a better fit for the data.

The results from the one, two and three class JLCMM's are presented below.

**XXX TABLES XXX**

**JLCMM NFL**

Again, a the results of a likelihood ratio test between two models was used to determine the random effects structure. Despite a non-significant test statistic of the likelihood ratio test ($\chi^2 = 1.38, p = 0.50$), a random effects structure with only intercept was chosen due to lower BIC for the simpler model ($BIC = 4077$) compared with the complex model ($BIC = 4088$).
