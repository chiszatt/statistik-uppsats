...
This study compared the predictive performance of different modeling frameworks for time-to-AD-dementia in individuals from the ADNI cohort, structured across three levels of information and their corresponding model approaches. At the most basic level, baseline biomarker information was used in Cox proportional hazards (PH) models estimated either with p-tau181 and NfL separately or jointly. The second level incorporated longitudinal biomarker information by extending these Cox PH models within a joint modeling framework that accounts for repeated measurements over time, while the most complex level combined longitudinal biomarker trajectories with latent class structure using the JLCMM framework to capture heterogeneity in disease progression and integrate class-specific trajectories with survival outcomes.

The predictive performance of these models was assessed by comparing time-dependent AUC and Brier scores with a landmark time of 1 year over horizons spanning 1-5 years. Both evaluations demonstrated that all three model approaches showed relatively good predictive performance, with AUC-values above 0.75 and low Brier score-values (below approximately 0.15) across the follow-up period.

### **Förslag på tolkning av AUC/Brier score results utifrån frågeställningarna**

At the same time, the time-dependent AUC showed that the predictive performance differed across the three levels of information considered. Overall, incorporating longitudinal biomarker trajectories improved discriminatory ability compared to relying on a single measurement such as baseline. This improvement was consistent across biomarkers (p-tau181, NfL, and their combination) and was most pronounced at early follow-up times, where joint models achieved the highest AUC values relative to Cox-based approaches. These findings are consistent with those of Goksuluk et al. (2025, p. 14), who reported improved predictive performance of joint modelling approaches compared to Cox PH models using, among other specifications, baseline biomarker values.

In this study, the corresponding time-dependent Brier scores additionally showed a broadly consistent but complementary pattern. Joint models generally improved predictive accuracy compared to Cox models over intermediate and longer horizons, reflecting better overall calibration when longitudinal information was incorporated.

However, while joint models showed the strongest performance at early follow-up according to the time-dependent AUC, latent class joint models achieved comparable or higher AUC values at intermediate to longer horizons. This indicates that there is no single modelling approach that consistently dominates in terms of time-dependent AUC across all time horizons, and even suggests a time-dependent shift in relative model performance.

Additionally, the time-dependent Brier score showed that latent class joint models demonstrated a marked improvement in short-term performance at 2 years, as indicated by substantially lower Brier scores. This suggests that different modelling frameworks captures distinct aspects of predictive performance. While joint models optimised short-term discriminatory ability, latent class joint models appeared to improve short-term absolute risk prediction and calibration. In other words, joint models performed better in correctly ranking individuals according to risk, while latent class structure contributed more to the accuracy of predicted probabilities in the short term.
