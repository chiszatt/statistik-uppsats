...
# Data preprocessing and descriptive analysi
...
## Processing of Examination Date and Baseline

For the 52 participants with two biomarker measurements recorded on the same examination date, minor discrepancies were observed depending on whether the first, last, or mean value was used. The mean value was selected as the primary approach, and sensitivity analyses showed no substantial differences in model precision.

Since the first biomarker examination dates occurred several years after the first available diagnostic examination dates, baseline was defined as the first biomarker examination date. This ensured that follow-up time was aligned with the onset of biomarker observation, and that all participants had comparable longitudinal trajectories from time zero. Participants who experienced dementia right at baseline were excluded from the survival analyses, as they did not contribute valid follow-up time.

However, in 82 observations, discrepancies were observed between p-tau181 and NfL examination dates within the same visit. Five cases exceeded 30 days, with differences of up to 231 days, indicating that these did not represent the same examination occasion. To address this, a common examination date was constructed using the midpoint between the two biomarker dates. Diagnostic records occurring more than 50 days prior to the first biomarker measurement were excluded to maintain temporal alignment between clinical and biomarker data.

Related to this, sensitivity analyses were also performed in which baseline was alternatively defined as either the earliest biomarker examination date or the first diagnostic visit. These analyses showed no meaningful differences in model precision, indicating robustness to the choice of time origin.
...
An initial comparison of model performance by assessment of the concordance indices for all cox models was performed. The results presented in **table ?** show that the concordance index evaluates all three models similarly.
...
The choice of random effects structure was limited due to the $JMBayes2$ package currently only supporting random effects structures including a random slope. This meant that all LME models were fit with a random intercept and slope. However, models with a simple structure with only a random intercept and an extended structure including a random slope with fit using maximum likelihood estimation in order to perform likelihood ratio tests. The results show that the extended structure was a better fit for NfL, but a simpler structure would have been preferable for ptau-181 (**appendix X**).
...
<!-- ![](figures/corrplot_cox.png){width="400"} -->

## Appendix X: correlation plot over the numerical variables used in the LME models

<!-- ![](figures/corrplot_lme.png){width="400"} -->

## Appendix X: results of the likelihood ratio tests to determine the random effects structure for LME models

```{r}
kable_standard(readRDS("tables/long_re_lrt_table.rds"))
```

## Appendix X: p-values for the proportional hazards assumption tests for all covariates in the Cox PH models

```{r, echo=FALSE}
kable_standard(readRDS("tables/table_ph.rds"))
```

## Appendix X: results of the likelihood ratio test for the Cox PH models with and without gender as a covariate
...
![](figures/convergence_plot_nfl.png){width="400"}

## Appendix X: trace plot of the $α$-parameter for the multivariate joint model

![](figures/convergence_plot_multi.png){width="400"}
