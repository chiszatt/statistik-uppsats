...
where $M_i$ represents the true unobserved biomarker profile up to time t. This model represents the linked time-to-event and disease process.

### Model Assumptions

A core assumption of the joint model for longitudinal and survival data is full conditional independence between the longitudinal outcome and the survival outcome. All dependence is assumed to be explained by the random effects, resulting in independence of the longitudinal and survival outcomes and the repeated biomarker measurements are independent of each other (Proust-Lima et al., 2018, pg. 62-63).
...
Time-dependent AUC is a measure of the model’s discriminatory ability at time $t$, i.e. its ability to distinguish between individuals who will experience the event after time $t$ and those who will not. It is defined via time-dependent sensitivity and specificity over a threshold $c \in [0,1]$ (Yang et al., 2026):
...
Varying $c$ yields the ROC curve, and the resulting time-dependent AUC summarizes the model’s ability to correctly rank individuals according to their risk over the prediction horizon $t+\Delta t$ (Yang et al., 2026). An AUC value of $1$ indicates perfect discrimination, whereas a value of $0.5$ corresponds to no discrimination, equivalent to random guessing (Rizopoulos, p. 196).

**Time-dependent Brier score**

The time-dependent Brier score measures the mean squared difference between the observed event status and the predicted event probability at time $t+\Delta t$, conditional on being event-free at time $t$ (Yang et al., 2026).

$$BS(t+\Delta t,t)=E\left[(I(T < t+\Delta t)-\Pi(t+\Delta t \mid t))^2 \mid T \ge t\right]$$

A lower value indicates better predictive performance. In practice, the Brier score is estimated using either a model-based approach or inverse probability of censoring weighting (IPCW) to account for censoring (Yang et al., 2026).
...
In the final model, baseline NfL was a significant predictor of disease progression (HR = 2.69, 95% CI: 1.90-3.82, p \< 0.001). This suggests that higher NfL levels at baseline are associated with increased risk of developing AD/dementia. APOE-$\epsilon 4$ status again showed a strong and consistent association, with a clear dose-response pattern (one allele: HR = 2.56; two alleles: HR = 5.19; both p \< 0.001). In contrast, neither age (HR = 1.01, p = 0.30) nor sex (HR = 1.00, p = 0.99) were statistically significant predictors in this model.
...
Next, a joint model was fitted to simultaneously analyse the longitudinal trajectory of p-tau181 and its association with time to progression to AD/dementia. A standard Cox model and a linear mixed-effects model were therefore first specified.
...
The association parameter linking longitudinal p-tau181 to the risk of progression was strongly positive (estimate = 1.371, 95% credible interval: 0.989-1.773, p \< 0.05). This indicates that higher p-tau181 levels over time are associated with a substantially increased hazard of developing AD/dementia.
...
Additionally, a joint model was fitted to analyse the longitudinal trajectory of NfL and its association with time to progression to AD/dementia.
...
Most importantly, the association between longitudinal NfL and risk of progression was strong and statistically significant (estimate = 1.649, 95% credible interval: 1.195-2.107, p \< 0.05). This indicates that higher NfL levels over time are strongly linked to increased hazard of developing AD/dementia.
...
- Complementary log-log plot showed curvature, Weibull assumes a linear log-log plot.

- Weibull baseline hazard was compared against a flexible baseline hazard using "3-quant-piecewise"

- BIC was lower for Weibull (5582) than for flexible hazard (5595), suggesting that Weibull fits the data better, despite slight violations of the log-log plot
