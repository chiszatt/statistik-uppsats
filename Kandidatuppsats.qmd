...
The Brier score can be thought of as the mean squared error of the prediction forecast, and is commonly formulated as:

$$\text{Brier Score} = \frac{1}{N}\sum_{t = 1}^{N}(p_t - o_t)^2$$

where $p_t$ is the predicted probability of an event and $o_t$ is the observed outcome. The Brier score takes values from 0 to 1, where a score closer to 0 is given to a model with better predictive ability (Seo Young Park, p. 1704). **\*\*TIME DEPENDENT BRIER SCORE\*\***
