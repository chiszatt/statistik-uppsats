...
**Class sizes are also important, as classes that are made up of less than** $5\%$ **of the total sample population may not be clinically meaningful (REF).**
...
**Diagnostic records occurring more than 50 days prior to the first biomarker measurement were excluded to maintain temporal alignment between clinical and biomarker data.**

Subsequently, sensitivity analyses were performed using alternative definitions of baseline, defined as either the earliest biomarker examination date or the first diagnostic examination date. No meaningful differences in model precision were observed, indicating robustness to the choice of time origin: **did we do this yet?**

## Division Into In-Sample And Out-of-Sample Datasets

**Additionally, the analyzed data were split so that the in-sample data set comprised three quarters of the total data and the out-of-sample comprised one quarter.**

**When restricting the analysis to participants with at least one biomarker measurement, the in-sample data set consisted of 701 individuals, among whom 145 experienced the event of dementia, corresponding to 20.69%, while 556 individuals were censored. The out-of-sample included 233 individuals, among whom 53 experienced the event of dementia, corresponding to 22.75%, while 180 individuals were censored.**
...
**ALSO: age at baseline not included in cox models, so we probably dont need to mention high correlation with NfL.**
...
Among the models including p-tau181, the time-dependent AUC values remain above 0.75 across all three modelling approaches throughout the follow-up period, indicating stable and relatively strong predictive performance overall. At earlier time points, the joint model (JM) provides the highest discrimination. However, the differences between the models decrease over time, and at the six-year follow-up, the latent class joint model (LCJM) slightly outperforms the other approaches. The Cox PH model consistently yields the lowest predictive performance across follow-up.

Among the different models including NfL, the same overall pattern is observed. As shown in Appendix X, all models including NfL maintain AUC values above 0.75 throughout the follow-up period, indicating consistently strong predictive performance. At earlier follow-up times, the joint model (JM) yields the highest AUC values. However, its performance gradually declines at later follow-up times, becoming more similar to the other models. Additionally, the latent class joint model (LCJM) achieves the highest AUC value at the six-year follow-up.

Appendix X, presenting the models including both biomarkers simultaneously, also demonstrates higher AUC values for the joint model at earlier follow-up time points. However, its AUC values gradually decline over time, yielding values more similar to those of the Cox PH model at later follow-up times.
...
By evaluating predictive performance using time-dependent Brier scores instead, the same overall patterns are observed, with lower values indicating better predictive accuracy. Figure X presents the time-dependent Brier score curves for the different modelling approaches, with the biomarker specification fixed to p-tau181.
...
According to the figure, the JLCM yields the lowest AUC values across the follow-up period. In comparison, at the earlier follow-up points, the two other models yield very similar AUC values. At the later
...
## Precision Analysis: Interval Censored Events
