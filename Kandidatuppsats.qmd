...
***Since there is evidence to support AD as a heterogeneous disease with differing disease trajectories (SOURCE), a JLCMM could be employed to assess whether individuals can be categorised into meaningful classes based on NfL and p-tau181 trajectories alongside survival data.***

***Is this relevent to us?:***

***Through the example of progression to multiple system atrophy (MSA), characterized by various combinations of progressive autonomic failure and motor dysfunction, the researchers identified five subphenotypes of MSA that differ by the sequence and shape of marker degradation, as well as the associated risk of death (Proust-Lima et al., p. ?).***

Various models have utilised longitudinal p-tau181 and NfL values with the aim of modeling AD progression, however, the question arises as to which of these modeling approaches has greatest predictive performance. Few studies have systematically compared the previously mentioned modeling approaches within a unified framework in AD research, specifically focusing on plasma biomarkers such as NfL and p-tau. **Frågan är även om NfL alltid ger bättre prediction än P-tau181, oavsett metod.**
...
The Cox PH model including both baseline biomarker values showed the highest predictive performance across all evaluated time points. In contrast, the single-marker models demonstrated slightly lower and somewhat more variable performance over time. The trajectories of the Cox NfL and Cox p-tau181 models were broadly similar, although the NfL-only model appeared to perform slightly better during later follow-up.
