...
Plotting the residuals against the fitted values for the LME model with p-tau181 showed no clear evidence of heteroskedasticity, as shown in Appendix 8.3.1. The slight deviations in the tails of the Q-Q plot were also not considered to be of concern (Appendix 8.3.2). A similar pattern was observed for the LME model of NfL, where the plot of residuals against fitted values demonstrated even less evidence of heteroskedasticity compared to the corresponding plot for p-tau181 (Appendix 8.3.3). The Q-Q plot for NfL showed similar results to those observed for p-tau181 (Appendix 8.3.4). Generalised LME models with different link functions were also fit and their residuals analysed, however none of these models demonstrated a better fit than the LME models with log-transformed data. Consequently, they were considered to provide an adequate fit to the data and as suitable for use within the joint modeling framework.
...
Among the models incorporating NfL, a similar overall pattern is observed. As shown in Appendix 8.6.1, the curves remain at comparable levels to those seen for the p-tau181 models, with time-dependent AUC values exceeding 0.75 throughout follow-up. At earlier follow-up times, where the values are slightly higher across models, the JM once again yields the highest AUC values. These values then decline over time, becoming more similar to those of the other models.

Appendix 8.6.2, presenting the models incorporating both biomarkers simultaneously, also demonstrates higher AUC values for the JM at earlier follow-up time points. These values gradually decline over time, yielding values more similar to those of the Cox PH model at later follow-up times.
...
A similar overall pattern is also observed among the models incorporating NfL (Appendix 8.6.3). The JLCMM yields lower Brier score values compared with the other two models at earlier follow-up points. Then, at later follow-up times, these values become smaller, resulting in the differences becoming smaller between the models, with values being low, stable and around the same range as for models including p-tau181.

The same can be said for the time-depending Brier score values among the models incorporating both biomarkers simultaneously. Appendix 8.6.4 demonstrates that the Cox PH model and the JM yield similar, stable AUC values across the follow-up period, ranging from approximately 0.05 at early follow-up to 0.14 at later follow-up.
...
Within the JM framework, a similar overall pattern is observed. More specifically, Appendix 8.7.1 demonstrates that the combined biomarker joint model provides the strongest predictive performance across all time points, with consistently higher AUC values than models including the biomarkers separately. Nevertheless, the NfL-based model generally shows slightly higher values than the p-tau181-based model.

Similarly, within the JLCMM framework, the model including NfL consistently shows slightly higher AUC values than the model including p-tau181, except at the three-year follow-up time point, where this model shows a slightly higher value (Appendix 8.7.2).

As when comparing the different modelling approaches, these results are also indicated by Table 10, containing the time-dependent AUC values for each biomarker specification and at each follow-up time point. For example, at the two-year follow-up within the JM framework, the model including the biomarkers separately achieved an AUC value of 0.967, compared to an AUC value of 0.953 for the model including NfL and 0.902 for the model including p-tau181. This indicates that the model provides the highest predictive performance at this time point.
