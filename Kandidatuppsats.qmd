...
Recent research has applied a range of statistical models to predict progression to dementia using biomarker data and evaluate their predictive accuracy. For instance, a 2021 study used a Cox proportional hazards (Cox PH) model with baseline p-tau181, alongside demographic and genetic covariates such as age, sex, and apolipoprotein E ε4 (APOE-ε4) status. The study included a total of 767 participants from the ADNI database, including cognitively unimpaired individuals, individuals with mild cognitive impairment (MCI), and individuals with dementia. Predictive performance was evaluated using receiver operating characteristic (ROC) analysis at a fixed prediction horizon, yielding an area under the curve (AUC) of 0.78. This suggests that the model demonstrated good discriminative ability in identifying individuals who later developed dementia during follow-up (Wang et al., 2021, pp. 1-2).

Similarly, a large population-based study of 2148 dementia-free older adults from Sweden applied a single multivariable Cox proportional hazards model including p-tau181, p-tau217, neurofilament light (NfL), and GFAP as simultaneous baseline predictors, to examine their predictive value for progression to all-cause and Alzheimer’s disease dementia. Predictive performance was assessed using a fixed 10-year prediction horizon, where the AUC was calculated based on dementia occurrence within 10 years. Within this framework, the presence of elevated values of one, two, or three biomarkers yielded AUC values of 74.3%, 77.0%, and 81.3% for AD-dementia, respectively, demonstrating progressively improved discriminative ability for identifying individuals who subsequently developed AD-dementia (Grande et al., 2025, pp. 2027-2031).
...
All of the modeling approaches considered in this study are grounded in the survival analysis framework. The primary aim is to predict the time until the occurrence of a pre-specified event of interest from some baseline time, usually the beginning of follow-up (Kleinbaum & Klein, 2012, p. 4). In this study, the event is defined as the first dementia diagnosis. Diagnosis status, ptau-181 and NfL levels are all available at baseline.
...
**Det som stod innan - har inte vävt ihop det ovan med detta**

At the 6 year mark, the differences in AUC between models were reduced, in line with expectations.

When comparing Brier scores, the short-term predictive performance of the JLCMMs were superior when compared against the other models.

The JLCMMs explained around 88% more of the variance than a null model which assigns all individuals the average event probability of 0.28. On the other hand, Cox PH models and JMs explained 72% and 73% more variance respectively.
...
However, modest differences in AUC were reported between the Cox PH models with single biomarker baseline values and the model using both p-tau181 and NfL at baseline, suggesting minimal increase in discriminative value over modeling either biomarker by itself within this modeling framework. Clearer distinctions were found for the JMs and JLCMMs, suggesting predictive value is gained mostly from NfL trajectories rather than single-point measurements.

The predictive performance of plasma p-tau181 and NfL were compared both individually and jointly by comparison of AUC and Brier scores. Across all modalities, the AUC for p-tau181 was lower than that for NfL at the 2 year mark. Modest differences in AUC were reported between the Cox PH models with single biomarker baseline values and the model using both p-tau181 and NfL at baseline, suggesting minimal increase in discriminative value over modeling either biomarker by itself within this modeling framework. However, clearer distinctions were found for the JMs and JLCMMs, suggesting predictive value is gained mostly from NfL trajectories rather than single-point measurements.

Over time, differences between the predictive performance of the biomarkers reduced, however NfL performed better across most time points. The multivariate JM presented the highest AUC at the 6 year mark, suggesting that this modeling framework may be justified when long-term discrimination between cases and non-cases are of importance. Comparison of Brier scores gave similar results between biomarkers.
...
# kable_standard(readRDS("tables/tables/results_table_AUC_last_duplicate.rds"))
```
...
#| tbl-pos: "H"
#| tbl-cap: "Time-depending Brier scores when visits with duplicate measurements are replaced by visits with the last measurement"
kable_standard(readRDS("tables/tables/results_table_Brier_last_duplicate.rds"))
```
